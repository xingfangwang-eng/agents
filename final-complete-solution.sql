-- Final complete solution for generating and inserting embeddings
-- Run this entire script in Supabase SQL Editor

-- Step 1: Enable pgvector extension (if not already enabled)
create extension if not exists vector;

-- Step 2: Create the embedding generation function
...
-- Step 5: Check some sample tools
select 
  slug, 
  name, 
  category, 
...
select 
  'All tools have been processed. Total tools: ' || count(*) || ', Tools with embeddings: ' || count(embedding)
from tools;